...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            delta_vector = np.subtract(waypoints[wp_index+1][:2],waypoints[wp_index][:2])
            delta_len = np.linalg.norm(delta_vector)
            arc_length += delta_len
            wp_index += 1
            if arc_length >= self._lookahead:
                break
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        current_wp = waypoints[i][:2]
        delta_vector = np.subtract(ego_state[:2], current_wp)
        current_len = np.linalg.norm(delta_vector)
