...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        #R = [a;b;c;d]
        # thetas = R'*[s;s**2/2;s**3/3;s**4/4]
        R = np.transpose(np.vstack((a,b,c,d)))
        s_vec = np.vstack((s,s**2/2,s**3/3,s**4/4))
        thetas = np.dot(R, s_vec)
        thetas = np.squeeze(thetas).tolist()
        return thetas
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        x_points = scipy.integrate.cumtrapz(np.cos(t_points), s_points).tolist()
        y_points = scipy.integrate.cumtrapz(np.sin(t_points), s_points).tolist()
