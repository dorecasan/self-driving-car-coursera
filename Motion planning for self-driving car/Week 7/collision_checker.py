    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = np.add(path[0][j], np.multiply(self._circle_offsets,cos(path[2][j])))
                circle_locations[:, 1] = np.add(path[1][j], np.multiply(self._circle_offsets,sin(path[2][j])))
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # --------------------------------------------------------------
                path = paths[i]
                end_path_point = np.array([path[0][-1],path[1][-1]])
                goal_point = np.array([goal_state[0],goal_state[1]])
                score = np.linalg.norm(goal_point-end_path_point)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # --------------------------------------------------
                            path_col = paths[j]
                            end_collision_path_point = np.array([path_col[0][-1],path_col[1][-1]])
                            score += self._weight *1/np.linalg.norm(end_path_point-end_collision_path_point)
                            #score += 1/np.abs(j-i)
                            # --------------------------------------------------
